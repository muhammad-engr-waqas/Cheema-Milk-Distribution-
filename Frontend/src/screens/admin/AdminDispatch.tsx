    ...
    <div className="space-y-6 max-w-full mx-auto"> <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-slate-200 shadow-sm"> <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2"> <Truck className="w-6 h-6 text-purple-600" /> Dispatch record
        ...
        > <Plus className="w-4 h-4" /> New dispatch
              ...
              {dispatches.map(d => {
                ...
                return (<tr key={d.id} className="hover:bg-slate-50 transition-colors"><td className="p-4"> <p className="font-bold text-slate-800">{d.date}</p> <p className="text-xs text-slate-500 mt-1 uppercase tracking-widest">{d.vehicleNumber}</p> </td> <td className="p-4"> <p className="font-bold text-slate-800">{d.roadName || '-'}</p> <p className="text-xs text-slate-500 mt-1">{d.trunkName || '-'}</p> </td> <td className="p-4"> <p className="font-bold text-slate-800">{d.driverName}</p> </td> <td className="p-4"> <div className="flex items-center text-xs w-32"> <strong className="font-mono text-indigo-600 font-bold text-sm bg-indigo-50 px-2.5 py-1 rounded border border-indigo-100">{totalSentLiters} L</strong> </div> </td> <td className="p-4 text-center"> <span className={`inline-block px-2.5 py-1 rounded text-xs font-bold ${
            ...
            > <Download className="w-3.5 h-3.5" /> PDF
              ...
              {filteredDispatches.filter(d => d.status === 'Received' || d.status === 'Completed').length > 0 ? (
                 filteredDispatches.filter(d => d.status === 'Received' || d.status === 'Completed').map((d, index) => {
                   ...
                   const diffFat = (Number(originalSum.avgFat) - Number(receivedSum.avgFat)).toFixed(2);
                   const tsLoss = (Number(originalSum.totalTsAmount) - Number(receivedSum.totalTsAmount)).toFixed(2);
                   return (<tr key={'hist-' + d.id} className="hover:bg-slate-50"><td className="p-3 font-mono text-center text-slate-400">{index + 1}</td> <td className="p-3 font-bold text-slate-700">{d.date}</td> <td className="p-3"> <div className="font-bold text-slate-800 text-xs">{d.vehicleNumber}</div> <div className="text-xs text-slate-500">{d.driverName}</div> </td> <td className="p-3"> <div className="flex flex-col gap-0.5 text-xs"> <div className="flex gap-2 justify-between"> <span className="text-slate-500 font-medium w-12">Liters:</span> <span className="font-mono font-bold text-indigo-700">{originalSum.totalMilkLiter} <span className="text-[9px]">L</span></span> </div> <div className="flex gap-2 justify-between"> <span className="text-slate-500 font-medium w-12">Kgs:</span> <span className="font-mono font-bold">{originalSum.totalMilkKgs} <span className="text-[9px]">kg</span></span> </div> <div className="flex gap-2 justify-between mt-1 pt-1 border-t border-slate-100"> <span className="text-slate-500 font-medium w-12">Fat:</span> <span className="font-mono font-bold">{originalSum.avgFat}%</span> </div> </div> </td> <td className="p-3 border-l border-slate-200 bg-emerald-50/10"> <div className="flex flex-col gap-0.5 text-xs"> <div className="flex gap-2 justify-between"> <span className="text-emerald-600 font-medium w-12">Liters:</span> <span className="font-mono font-bold text-emerald-700">{receivedSum.totalMilkLiter} <span className="text-[9px]">L</span></span> </div> <div className="flex gap-2 justify-between"> <span className="text-emerald-600 font-medium w-12">Kgs:</span> <span className="font-mono font-bold text-emerald-700">{receivedSum.totalMilkKgs} <span className="text-[9px]">kg</span></span> </div> <div className="flex gap-2 justify-between mt-1 pt-1 border-t border-emerald-100/50"> <span className="text-emerald-600 font-medium w-12">Fat:</span> <span className="font-mono font-bold text-emerald-700">{receivedSum.avgFat}%</span> </div> </div> </td> <td className="p-3 border-l border-slate-200 bg-red-50/30"> <div className="flex flex-col gap-0.5 text-xs"> <div className="flex gap-2 justify-between"> <span className="text-red-500 font-medium w-12">Liters:</span> <span className="font-mono font-bold text-red-600">{literLoss} <span className="text-[9px]">L</span></span> </div> <div className="flex gap-2 justify-between"> <span className="text-red-500 font-medium w-12">Kgs:</span> <span className="font-mono font-bold text-red-600">{kgLoss} <span className="text-[9px]">kg</span></span> </div> <div className="flex gap-2 justify-between mt-1 pt-1 border-t border-red-100/50"> <span className="text-red-500 font-medium w-12">Fat:</span> <span className="font-mono font-bold text-red-600">{diffFat}%</span> </div> </div> </td></tr>
                   );
                 })
